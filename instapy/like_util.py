...
#sc added
def get_user_score(browser, user_to_check,logger):
    #get some info about the user.  A score of follower / following.
    user_to_check_link = 'https://www.instagram.com/' + user_to_check
    browser.get(user_to_check_link)

    number_followers = browser.execute_script(
        # sc edit 3/3/2018
        # "return window._sharedData.""entry_data.ProfilePage[0]."
        # "user.followed_by.count")
        "return window._sharedData.entry_data.ProfilePage[0].graphql.user.edge_followed_by.count")
    logger.info('number_followers='+str(number_followers))


    number_following = browser.execute_script(
        # sc edit 3/3/2018
    ...
    logger.info('number_following='+str(number_following))

    ret = 0
    if number_following > 0:
        ret = (float(number_followers)/float(number_following)*100)
        ret = round(ret,0)
     
    logger.info('user_score='+ str(ret))            
    return ret
...
def check_link(browser,
    ...
    if like_by_followers_upper_limit or like_by_followers_lower_limit:
	    ...
	    #sc added this.  there may be better spot for this function call ----------------------------------------------
        #it calls my function to get a user score.  I plan to use the user score later to help determine who to follow.
        get_user_score(browser, user_name, logger)
    ...
    for dont_likes_regex in dont_like_regex:
        ...
        if quash:
            ...
            quashed = quashed.encode('utf-8').strip()#maybe like this?       .encode('utf-8').strip()
            iffy = (re.split(r'\W+', dont_likes_regex))[3]
            inapp_unit = ('Inappropriate! ~ contains \'{}\''.format(quashed) if quashed == iffy else
							'Inappropriate! ~ contains \'{}\' in \'{}\''.format(iffy, quashed))  #this line causes the program to crash if there is a unicode character such as the crown emoji
